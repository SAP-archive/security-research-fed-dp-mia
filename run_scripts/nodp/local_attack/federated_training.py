...
def bootstrap_instance(seed, i, output_size, connection_string):
    experiment_name = f"{model}{output_size}_s_{seed}"
    ...
    output = f"./experiments/local/{experiment_name}"
    ...
    start_mi_command = " ".join(
        ["nohup", "python", f"./FIA/run_scripts/nodp/local_attack/train_{inference}.py", "--path", output,
    ...
    execute_command_wait(
        ["rsync", "-amv", "--stats", "--exclude", "texas*", "--exclude", ".git", "--exclude", ".fuse*", "--exclude",
         "*.h5", "--exclude",
         "*.hdf5",
    ...
    execute_command(
        ["ssh", "-o", "StrictHostKeyChecking=no", "-i", key, connection_string,
         ...
         f"sleep 180;"
         f"{start_mi_command} >./logs/mia{i}.txt;"
         ])
...
for seed in seeds:
    for output_size in output_sizes:
        p = multiprocessing.Process(target=bootstrap_instance, args=(seed, i, output_size,
