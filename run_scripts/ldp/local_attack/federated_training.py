...
def bootstrap_instance(seed, i, output_size, e, connection_string):
    experiment_name = f"{model}{output_size}_ldp_{e}_s_{seed}_orig"
    if model == "texas" or model == "purchases":
        dataset = f"./data/shokri_{model}_{output_size}_classes_eps_{e}.npz"
        dataset_orig = f"./data/shokri_{model}_{output_size}_classes.npz"
    else:
        dataset = f"./data/{model}_{output_size}_classes_eps_{e}.npz"
        dataset_orig = f"./data/{model}_{output_size}_classes.npz"
    output = f"/mnt/efs/fia/experiments/local/{experiment_name}"
    start_server_command = " ".join(
        ["nohup", "python", "-m",
         ...
         "4",
         "--epochs", "1", "--experiment", experiment_name,
         "--isServer", "--learning_rate", str(target_lr),
         "--alternative_dataset", dataset,
         "--output_size", str(output_size), "--seed", str(seed), "--port", str(5000 + i)])
    start_clients_command = " ".join(
        ["nohup", "python", "-m",
         "libs.FederatedFramework.core.experiments.run_script", "--output",
         f"{output}/target", "--model", model, "--batch_size", str(target_b), "--save_epochs", "5",
         "--hasAdversaryClient",
         "--clients", str(clients), "--experiment", experiment_name, "--isClient", "--learning_rate", str(target_lr),
         "--alternative_dataset", dataset,
         "--output_size", str(output_size), "--seed", str(seed), "--port", str(5000 + i)])
    start_mi_command = " ".join(
        ["nohup", "python", f"./FIA/run_scripts/nodp/local_attack/train_{inference}.py", "--path",
         output,
         "--dataset", dataset_orig, "--output_size", output_size, "--seed", str(seed), "--batch_size", str(attack_b),
    ...
    execute_command(
        ["ssh", "-o", "StrictHostKeyChecking=no", "-i", key, connection_string,
         ...
         f"sleep 180;"
         f"{start_mi_command} >./logs/mia{i}_{output_size}_{e}.txt;"
         ])
    print(f"training initiated on {connection_string}")
...
for seed in seeds:
    for e in eps:
        for output_size in output_sizes:
            p = multiprocessing.Process(target=bootstrap_instance, args=(seed, i, output_size, e,
