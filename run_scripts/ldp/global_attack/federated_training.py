...
def bootstrap_instance(seed, i, output_size, e, connection_string):
    ...
    start_clients_command = " ".join(
        ["nohup", "python", "-m",
         ...
         str(target_b), "--clients", str(clients), "--experiment", experiment_name, "--isClient", "--learning_rate", str(target_lr),
         "--alternative_dataset", dataset,
         "--output_size", str(output_size), "--seed", str(seed), "--port", str(5000 + i)])
    start_mi_command = " ".join(
        ["nohup", "python", f"./FIA/run_scripts/nodp/global_attack/train_{inference}.py", "--path", output,
    ...
    execute_command(
        ["ssh", "-o", "StrictHostKeyChecking=no", "-i", key, connection_string,
         ...
         f"sleep 180;"
         f"{start_mi_command} >./logs/mia_{output_size}_{e}.txt;"
         ])
